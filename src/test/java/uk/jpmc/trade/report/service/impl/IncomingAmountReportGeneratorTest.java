...
public class IncomingAmountReportGeneratorTest {

	private IncomingAmountReportGenerator incomingAmountReportGenerator;
	
	@Before
	public void setup() {
		incomingAmountReportGenerator = new IncomingAmountReportGenerator();
	}
	
	@Test
	public void testGenerateReportShouldReturnEmptyReportContentForNullOrEmptyInstructions() {
		StringBuilder reportContent = incomingAmountReportGenerator.generateReport(null);
		assertTrue(reportContent.length() == 0);
		
		reportContent = incomingAmountReportGenerator.generateReport(new ArrayList<>());
		assertTrue(reportContent.length() == 0);
	}
	...
	public void testGenerateReportShouldReturnNonEmptyReportContentForValidInstructions() {
		...
		StringBuilder reportContent = incomingAmountReportGenerator.generateReport(tradeInstructions);
		assertTrue(reportContent.length() >= 0);
	}
