...
public class IncomingAmountReportGenerator implements IReportGenerator {
	...
	public StringBuilder generateReport(List<TradeInstruction> tradeInstructions) {
		...
		if (tradeInstructions != null && tradeInstructions.size() >= 1) {
			...
			reportContent.append("    Daily Incoming AMOUNT SETTLEMENT Report" + TradeReportConstrants.NEWLINE);
			reportContent.append(TradeReportConstrants.REPORT_SEPERATOR + TradeReportConstrants.NEWLINE);
			reportContent.append(TradeReportConstrants.REPORT_HEADER + TradeReportConstrants.NEWLINE);
	
			Map<LocalDate, BigDecimal> reportData = ReportGeneratorUtil.calculateDailyAmount(tradeInstructions, Predicates.INCOMING_PREDICATE);
			reportData.forEach((k, v) -> reportContent.append(k).append("       |      ").append(v).append("\n"));
		}
